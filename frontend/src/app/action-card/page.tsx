...
import { getActionCards, updateActionCardStatus, updateActionCard, convertActionCardToOrder, deleteActionCard } from '@/lib/api';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-4 lg:max-h-[calc(100vh-200px)] lg:overflow-y-auto pr-3">
          {isLoading ? (
            <div className="py-12 text-center text-sm text-slate-400">Loading cards...</div>
          ) : error ? (
              ...
              >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="relative">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setOpenMenuCardId(openMenuCardId === card.id ? null : card.id);
                        }}
                        className="p-1 hover:bg-slate-100/80 rounded text-slate-400 hover:text-slate-700 transition-colors cursor-pointer"
                        title="More actions"
                      >
                      ...
                      {openMenuCardId === card.id && (
                        <>
                          ...
                          />
                          <div className="absolute right-0 mt-1 w-28 bg-white border border-slate-200 rounded-lg shadow-lg py-1 z-20 text-[11px]">
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-slate-700 font-bold transition-colors cursor-pointer flex items-center gap-1.5"
                            >
                            ...
                            {['pending', 'rejected'].includes(card.status?.toLowerCase()) ? (
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="w-full text-left px-3 py-1.5 hover:bg-red-50 text-red-600 hover:text-red-700 font-bold transition-colors cursor-pointer flex items-center gap-1.5 border-t border-slate-100"
                              >
                              ...
                              >
                                <svg className="h-3.5 w-3.5 text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                                Delete
                              </button>
                ...
                <div className="mt-2 pt-2 border-t border-slate-100">
                  ...
                  <ul className="space-y-0.5">
                    {card.items.map((item, idx) => (
                      <li key={idx} className="text-xs text-slate-600 flex justify-between font-semibold">
                ...
                <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-600 gap-4">
                  ...
                  <span className="flex items-center gap-1 max-w-[42%] truncate shrink-0">
                    ...
                    {isDayMissing(card) && (
                      <span className="text-amber-600 font-black text-xs cursor-help shrink-0" title="Delivery day not specified">⚠</span>
                    )}
                  </span>
                </div>
              </div>
            ...
            isEditing ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  ...
                  <span className="text-xs font-mono text-slate-400 font-bold">{selectedCard.id}</span>
                </div>
                      ...
                      {editItems.map((item, idx) => (
                        <div key={idx} className="flex gap-2 items-center">
                          ...
                          <button
                            ...
                            className="text-slate-400 hover:text-red-600 disabled:opacity-35 cursor-pointer font-extrabold text-sm"
                          >
                            &times;
                          </button>
                        </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex gap-2">
                  ...
                  <button
                    onClick={saveEdit}
                    className="flex-1 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-bold cursor-pointer"
                  >
                    Save Changes
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-[1.2fr_0.8fr] gap-4 mt-1.5 items-center">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-xs text-indigo-600 font-bold truncate">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-[1.2fr_0.8fr] gap-4 pb-2 border-b border-slate-100 text-xs">
                    <div className="flex items-center gap-2 min-w-0">
                  ...
                  {selectedCard.transcript && (
                    <div className="pt-4 border-t border-slate-100">
                      <button
                        ...
                        className="w-full flex items-center justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider py-2 px-3 bg-white border border-slate-200 hover:bg-slate-50/50 hover:border-slate-300 rounded-lg transition-all"
                      >
                      ...
                      {isAiAnalysisExpanded && (
                        <div className="mt-3 p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-4">
                          ...
                          <div>
                            <blockquote className="text-xs text-slate-600 bg-white p-2.5 rounded border border-slate-200 italic leading-relaxed font-medium">
                              "{selectedCard.transcript}"
                            </blockquote>
                          </div>
                        </div>
                ...
                <div className="pt-4 border-t border-slate-100 space-y-2">
                  ...
                  {getCardValidationWarning(selectedCard) && (
                    <div className="text-sm text-amber-700 font-semibold flex items-center justify-start text-left gap-2 py-1 my-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:hover:bg-red-600 text-white rounded text-xs font-bold cursor-pointer transition-colors shadow-xs"
                    >
                      Reject
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-md w-full p-6 space-y-4 animate-in fade-in-50 zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-extrabold text-slate-900">Delete Action Card</h3>
                <p className="text-xs text-slate-500 font-medium leading-relaxed">
                  Are you sure you want to permanently delete the Action Card for <strong className="text-slate-800 font-semibold">{deleteConfirmCard.customer_name || 'Anonymous'}</strong>? This action cannot be undone.
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 border border-slate-300 hover:bg-slate-50 rounded-lg text-xs font-bold text-slate-700 cursor-pointer transition-all shadow-3xs"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={async () => {
                  const cardId = deleteConfirmCard.id;
                  setDeleteConfirmCard(null);
                  try {
                    await deleteActionCard(cardId);
                    // Reload list
                    await loadCards();
                    // Clear selected card if we deleted it
...
export default function ActionCardPage() {
  return (
    <Suspense fallback={<div className="py-12 text-center text-sm text-slate-400 font-bold">Loading action cards...</div>}>
      <ActionCardContent />
    </Suspense>
  );
}
