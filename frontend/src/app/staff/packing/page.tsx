      ...
      if (!isSilent) {
        ...
        if (token) {
          ...
          try {
            ...
            router.push('/login?redirectTo=/staff/packing');
            return;
          }
        }
      }
          ...
          {orders.map(order => (
            <div key={order.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
              ...
              <button
                ...
                className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold transition-colors mt-4 cursor-pointer"
              >
