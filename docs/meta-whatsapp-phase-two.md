...
The phase-one all-history `(provider, phone_number_id)` constraint is replaced
with partial unique indexes for live connections. This preserves disconnected
history, allows a legitimate reconnect, and still prevents a phone number,
WABA, or shop from having more than one live PhoneERP connection.
