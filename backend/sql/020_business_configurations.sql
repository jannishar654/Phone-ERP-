...
-- Current application behavior assumes exactly one shop per owner. Fail the
-- migration clearly if historical duplicates need manual reconciliation.
DO $$
BEGIN
    IF EXISTS (
        SELECT owner_id FROM public.shops GROUP BY owner_id HAVING COUNT(*) > 1
    ) THEN
        RAISE EXCEPTION
            'Cannot enforce one shop per owner: duplicate shops.owner_id rows exist';
    END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS idx_shops_owner_id_unique
