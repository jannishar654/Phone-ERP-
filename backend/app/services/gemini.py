...
def aggregate_items_deterministically(normalized_items: list[dict]) -> list[dict]:
    """Aggregate identical items without collapsing restaurant variants."""
    aggregated_items = {}
    for item in normalized_items:
        ...
        normalized_add_ons = tuple(
            sorted(str(add_on).strip().lower() for add_on in add_ons if str(add_on).strip())
        )
        key = (
            name,
            unit_lower,
            str(item.get("size_variant") or "").strip().lower(),
            normalized_add_ons,
            str(item.get("spice_level") or "").strip().lower(),
            str(item.get("veg_non_veg") or "").strip().lower(),
        )
